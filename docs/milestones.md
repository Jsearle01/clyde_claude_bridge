...
| (T-P2-004 followup #2) | T-P2-004.6 | COMPLETE, awaiting verdict — `shell: true` on Windows for CVE-2024-27980. T-P2-004.5's explicit candidate iteration didn't resolve the defect: Node's CVE-2024-27980 patch (April 2024; Node ≥ 18.20.0) makes spawn/spawnSync return EINVAL for `.cmd` and `.bat` files on Windows unless `shell: true` is set, regardless of whether the name is bare, explicit, or absolute. Fix: add `shell: process.platform === "win32"` to both `spawnSync` (probe) and `spawn` (CLI start) call sites in `daemon-lifecycle.ts`. Both calls accompanied by an inline comment referencing the CVE. Candidate iteration from T-P2-004.5 retained as defensive belt-and-suspenders. One test assertion updated from strict `{stdio: "ignore", shell: false}` to `expect.objectContaining({stdio: "ignore"})` so the platform-conditional `shell` value doesn't fail across hosts. Tests 468/468 passing + 15 skipped (unchanged count; shape of one assertion adjusted). `.vsix` at 12.27 KB. One new v0.6 candidate logged (C-22 orchestrator-narrowing-audit when fix #1 fails); C-20 wording refined to name the CVE as the load-bearing reason. |
