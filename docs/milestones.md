...
| P2 Phase 4 | T-P2-004 | COMPLETE, awaiting verdict — daemon-lifecycle palette command + SecretStorage API key. New `daemon-lifecycle.ts` module (Option A spawn: extension invokes `claude-bridge start` CLI subcommand; all daemon-spawn logic stays in CLI). `locateCliBinary()` pure helper (PATH probe + `claudeBridge.cliPath` override); `getApiKey()` env→SecretStorage→showInputBox prompt chain (key `claudeBridge.anthropicApiKey`); `startDaemon()` orchestrator with 5s observation window for immediate-failure detection (stderr-parsed `already_running` vs generic `spawn_failed`). Extension wires "Claude Bridge: Start Daemon" command + `claudeBridge.autoStartDaemon` setting (one-shot on activate's first failed connect). Mock extended with `showInputBox` + `secrets` (PromiseLike-typed to satisfy VS Code's `SecretStorage.get` Thenable return) + `getConfiguration`/`makeWorkspaceConfig` helper. Tests 465/465 passing + 15 skipped (+15 new: 4 locateCliBinary + 5 getApiKey + 6 startDaemon). .vsix repackaged at 11.68 KB (was 8.34); reinstalled on Windows. No daemon-side changes. |
