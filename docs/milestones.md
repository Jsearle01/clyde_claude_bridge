...
| (T-P2-004 followup) | T-P2-004.5 | COMPLETE, awaiting verdict — Windows `.cmd` shim resolution. T-P2-004's `locateCliBinary` probed bare `claude-bridge` via `spawnSync`, which on Windows didn't auto-resolve PATHEXT in the VS Code extension host; CLI was reachable from PowerShell but the extension probe failed. Fix: F2 explicit-candidate iteration. `CLI_CANDIDATES = ["claude-bridge.cmd", "claude-bridge.exe", "claude-bridge"]` on Windows; `["claude-bridge"]` on Unix. `locateCliBinary` returns the first candidate where `spawnSync(name, ["--version"])` returns status 0. `CliBinaryNotFoundError.searchedPaths` renamed → `searchedNames` with updated message listing all tried candidates + `claudeBridge.cliPath` hint. `startDaemon` already used the resolved name (no change). Tests 468/468 passing + 15 skipped (+3 net: existing T-P2-004 tests adjusted to inject explicit candidates list and the renamed field; 3 new Windows-shim tests for cmd-hit, fall-through-to-exe, all-fail-with-full-list). `.vsix` repackaged at 12.01 KB. Two v0.6 candidates logged: C-20 (Windows `.cmd` shim pattern for future spawn sites), C-21 (mock-vs-production contract drift on cross-platform child_process). |
