...
| AC-3 | Claude.ai project calls `ping(message='hello')` and receives correct response | IMPLEMENTED | T-0011 (`mcp/dispatch.ts`, `mcp/tools/ping.ts`) | **Visual gate; human verifies.** Acceptance test §3; integration test 17.b verifies; end-to-end gate test from a real Claude.ai project at T-0019 |
...
| AC-5 | Successful ping produces audit log entry `allowed:true, tool:"ping"` with non-zero duration_ms | IMPLEMENTED | T-0011 (`mcp/dispatch.ts` `ToolRegistry.invoke` audit-write path) | Acceptance test §5; integration test 17.c verifies; end-to-end audit-file inspection at T-0019 |
