...
| AC-2 | `claude-bridge status` reports `Daemon: up` and `Tunnel: up` | IMPLEMENTED | T-0016 (`cli/src/commands/status.ts`, `cli/src/index.ts`) | Acceptance test §2; unit tests on formatUptime/formatBytes/collapsePath/formatStatusPayload + integration tests against real IpcServer (PID absent/stale → down; PID alive + status_ok → formatted block); end-to-end smoke validation at T-0019 |
...
| AC-6 | Killing cloudflared respawns within 30s with new URL; status reflects new URL | IMPLEMENTED | T-0012 (`tunnel/manager.ts` exit-handler triggers respawn; emits url_change on new URL) | Acceptance test §6; manager.test.ts 15.c/15.d verify mechanism; end-to-end with real cloudflared at T-0019. Manual recovery from `degraded` state covered by `claude-bridge tunnel restart` (T-0017). |
| AC-7 | `claude-bridge stop` cleanly shuts down both processes, removes PID file, flushes audit log | IMPLEMENTED | T-0016 (`cli/src/commands/stop.ts`, `cli/src/index.ts`) + T-0008 (IPC stop handler) + T-0013 (daemon main reverse-instantiation shutdown) | Acceptance test §7; CLI-side unit tests cover PID absent/stale/alive+success/alive+connection-error/alive+timeout; daemon-side shutdown sequence smoke-validated 2026-05-22 (SMOKE-3, 14ms reverse-instantiation); end-to-end at T-0019 |
| AC-8 | `claude-bridge token rotate` generates new token, invalidates old (verified 401), prints new | IMPLEMENTED | T-0017 (`cli/src/commands/token.ts`, `cli/src/index.ts`) + T-0013 (daemon tokenRotate handler mutates the `getExpectedToken` thunk) + T-0010 (auth middleware reads through the thunk every request) | Acceptance test §8; cli-side unit tests cover PID absent/stale/alive+success/alive+connection-error/alive+timeout; old-token-invalidation invariant enforced by daemon design (auth.test.ts) and verified end-to-end at T-0019 (rotate → old token 401 → new token 200) |
