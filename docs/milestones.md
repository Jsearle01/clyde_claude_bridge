...
| P2 Phase 6 | T-P2-006 | COMPLETE, awaiting verdict — status bar item with workspace identifier + daemon state. Added `IpcClient.onStateChange` + `WorkspaceRegistration.onStateChange` callback fields via private `setState` helpers (refactored 5 IpcClient + 13 Registration scattered `this.state = X` writes; idempotent no-ops, subscriber errors swallowed — same pattern as T-P2-005's `onReconnectAttempt`). New `packages/extension/src/status-bar.ts` (~165 lines): `makeStatusBar(sources, deps?)` factory + pure `composeStatusBarText` + `composeStatusBarTooltip` helpers; right-aligned item with state-driven icon ($(plug)/$(circle-slash)/$(sync~spin)/$(alert)/$(question)/$(warning)) + identifier; MarkdownString tooltip with workspace path, identifier, trust, daemon, URL; hidden when no workspace folder open. New `packages/extension/src/status-bar-menu.ts` (~190 lines): state-dependent QuickPick (Show Status always; Start Daemon when disconnected; Open Daemon URL + Copy Identifier when connected+registered; descriptive Stop hint per Q3c; trust_denied/duplicate descriptive items). Extension wires both onStateChange callbacks to `statusBar.refresh()`; registers `claudeBridge.openStatusBarMenu` command. Mock extended with `StatusBarAlignment`, `StatusBarItem` interface + factory, `createStatusBarItem`, `MarkdownString` class, `showQuickPick`, `env.clipboard.writeText`, `Uri.parse`. Tests 516/516 passing + 15 skipped (+35 new T-P2-006: 17 status-bar + 12 status-bar-menu + 3 IpcClient onStateChange + 3 Registration onStateChange). `.vsix` at 17.88 KB / 11 files (was 13.51/9). Third instance of "settable single-subscriber callback field" pattern; promotion to project pattern doc deferred to follow-up housekeeping task. No new v0.6 candidates. |
