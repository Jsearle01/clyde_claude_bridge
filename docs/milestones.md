...
| P2 Phase 3 | T-P2-003 | COMPLETE, awaiting verdict — workspace registration end-to-end. 3 new request kinds (`register_workspace`, `confirm_trust`, `deregister_workspace`) + 3 new response kinds in shared/ipc.ts; `WorkspacesStore` (file-backed `~/.claude-bridge/workspaces.json`) + pure `generateIdentifier()` (slug + 6-hex suffix, collision retry) in daemon/workspace/; daemon IpcServer gains active-registry `Map<abs_path, ActiveRegistration>` + 3 new handlers + socket-close cleanup; extension's IpcClient extended with `request<R>()` single-flight method + post-hello data-handler multiplexing; new extension modules `registration.ts` (`WorkspaceRegistration` class with full state machine) + `trust-prompt.ts` (modal wrapper). P1 stub `packages/daemon/src/workspace/registry.ts` UNTOUCHED per AC-15. Tests 450/450 passing + 15 skipped (was 421; +29 new: 11 identifier + 7 store + 5 daemon ipc handlers + 6 extension registration). .vsix repackaged at 8.34 KB (was 4.96); reinstalled on Windows. |
