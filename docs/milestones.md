...
| P2 Phase 5 | T-P2-005 | COMPLETE, awaiting verdict — daemon-not-running detection UX. New `IpcClient.onReconnectAttempt` callback field (single-subscriber pattern, fires in `scheduleReconnect()` after counter increment, subscriber errors swallowed). `runStartDaemonCommand` lifted from `extension.ts` to `daemon-lifecycle.ts` at the second-use moment; all three call sites (palette, autoStart, new notification button) share. New `makeDaemonNotRunningHandler(context, deps?)` factory in `daemon-lifecycle.ts`: closure-local once-per-session guard naturally resets per activate; threshold 3 attempts ≈7s wall-clock; reads `claudeBridge.autoStartDaemon` at notification-time; suppresses when true; surfaces `showWarningMessage` with `[Start Daemon]` button that invokes the extracted helper. Tests 481/481 passing + 15 skipped (+13 new T-P2-005: 3 callback firing, 3 runStartDaemon UI binding, 7 handler threshold/suppression). `.vsix` at 13.51 KB. No new v0.6 candidates. |
