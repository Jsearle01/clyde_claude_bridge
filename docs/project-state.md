...
| C-20 | T-P2-004 manual verification + T-P2-004.5 fix | Windows `.cmd` shim resolution: `child_process.spawn`/`spawnSync` on Windows doesn't reliably auto-resolve bare-name CLI shims via PATHEXT in the VS Code extension host. Affects any spawn of npm-installed CLI binaries. Pattern to codify: platform-aware explicit-candidate iteration (`['x.cmd', 'x.exe', 'x']` on Windows, `['x']` on Unix), not `shell: true`. | open |
...
### In progress
- T-P2-004.5 — Windows `.cmd` shim resolution for `claude-bridge` CLI (COMPLETE, awaiting verdict)

### Pending
- T-P2-005 — Phase 5 daemon-not-running detection UX + actionable retry notification
