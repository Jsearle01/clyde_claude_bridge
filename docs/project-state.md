...
| C-29 | T-P2-008.5 operator-side AC-24 retry, 2026-05-27 | Extension stuck at "registering" state after T-P2-008.5 .vsix bundling fix. Activation event fires; daemon healthy; no inbound IPC connection observed daemon-side; no extension-side diagnostics available (no output channel, no console logs). Possible root causes: (i) T-P2-008's IPC client additions interact poorly with registration; (ii) CJS bundle from T-P2-008.5 changed module init order; (iii) named-pipe resolution failure. Status: open; diagnostics added in T-P2-008.6; fix in subsequent task informed by diagnostic output. Blocks AC-24. **Update (T-P2-008.6 operator smoke):** modals fired in R1/R2/R3a — extension connects + registers + relays approval responses correctly, so the "stuck at registering" symptom did not reproduce on the post-diagnostics run. C-29 appears resolved; pending orchestrator confirmation. | open (blocks AC-24) → likely resolved |
...
### In progress
- T-P2-008.7 — C-30 session_bypass fix (gate guard + session-keyed bypass) (COMPLETE, awaiting verdict)
...
- AC-24 retry (R3a + R3b) — operator runtime smoke (C-25.c) for T-P2-008.7; if R3b modal does NOT re-fire → C-30 closed, AC-24 closed, P2 critical path resumes (T-P2-006-followup, then T-P2-009)
