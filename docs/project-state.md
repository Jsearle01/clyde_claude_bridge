...
### In progress
- T-P2-006 — Phase 6 status bar item with workspace identifier + daemon state (COMPLETE, awaiting verdict)

### Pending
- T-P2-006-followup — small housekeeping task: promote "settable single-subscriber callback field" pattern to `docs/patterns/project/settable-single-subscriber-callback.md` (third confirmed use now: `onReconnectAttempt` + `IpcClient.onStateChange` + `WorkspaceRegistration.onStateChange`)
- T-P2-007 — Phase 7 workspace registry replacement (swap P1 stub for workspaces.json-backed real registry on the daemon side)
