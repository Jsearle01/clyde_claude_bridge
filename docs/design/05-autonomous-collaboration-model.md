...
- **T-P3-003 (consent modal — not started, was next):** becomes "**bind this workspace + (optionally) set granularity**," not just approve/deny. The modal communicates *what workspace is being bound*, not just "allow claude.ai."
- **T-P3-004 (/token + auth layer — not started):** the token record carries **`bound_workspace` + `granularity`**; the auth layer **enforces the binding** (reject tool calls targeting a non-bound workspace) and **exposes the granularity** to the gate.
- **New work — gate re-key:** the approval gate (`gate.ts`), currently workspace-keyed, re-keys to the **token/binding** and consults the per-operation granularity; resolve the `session_bypass` vestigial ambiguity here.
