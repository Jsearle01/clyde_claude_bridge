...
- **Windows detached subprocess discipline.** When spawning detached children on Windows, set `windowsHide: true` to suppress unwanted console windows (Node spawn option; no-op on Unix). Additionally, parent processes that spawn detached daemons must avoid redirected stdio for the launching command — the redirect file handle inherits to the detached grandchild and pins the parent process until the daemon exits. Use fire-and-forget patterns (`> NUL 2>&1` at shell level, or capture daemon state via IPC instead of stdout) when launching daemons from PowerShell. Both findings emerged from T-0015 + T-0019 calibration; codified at T-0019.5.
